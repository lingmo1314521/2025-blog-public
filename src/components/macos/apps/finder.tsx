    ...
    <div className="flex h-full w-full bg-[#f6f6f6] dark:bg-[#1e1e1e] text-black dark:text-white rounded-b-xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
      <div className="w-48 bg-black/5 dark:bg-white/5 backdrop-blur-xl border-r border-black/5 dark:border-white/5 flex flex-col pt-4 px-2 gap-6">
        <div className="flex flex-col gap-1">
          <div className="px-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">{t('favorites')}</div>
          <SidebarItem icon={Home} label={t('all_posts')} active={activeSidebar === 'all_posts'} onClick={() => setActiveSidebar('all_posts')} />
          <SidebarItem icon={Clock} label={t('recent')} active={activeSidebar === 'recent'} onClick={() => setActiveSidebar('recent')} />
          <SidebarItem icon={FileText} label={t('drafts')} />
        </div>
        <div className="flex flex-col gap-1">
          <div className="px-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">{t('locations')}</div>
          <SidebarItem icon={Cloud} label={t('icloud')} />
          <SidebarItem icon={Download} label={t('downloads')} />
        </div>
      </div>
      <div className="flex-1 flex flex-col h-full bg-white dark:bg-[#1e1e1e]">
        ...
        <div className="h-6 border-t border-black/5 dark:border-white/10 flex items-center justify-center text-[10px] text-gray-400 select-none">{posts.length} {t('items')}</div>
      </div>
    </div>
  )
}