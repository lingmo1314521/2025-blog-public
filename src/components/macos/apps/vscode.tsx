...
import { 
  ...
  FileCode, AlertCircle, CheckCircle2, Terminal, 
...
export const VSCode = () => {
  ...
  const [initialSelectedIds, setInitialSelectedIds] = useState<string[]>([]) // Store selection state before drag start
  ...
  useEffect(() => { localStorage.setItem('vscode-fs-v8', JSON.stringify(fileSystem)) }, [fileSystem])
  ...
  const handleItemClick = (e: React.MouseEvent, id: string, type: FileType) => {
    ...
    // Toggle Folder on click if not dragging selection
    ...
    if (e.ctrlKey || e.metaKey) {
        ...
        // Range select could be implemented by flattening list, but simple multi-add is safe for now
  ...
  // --- Box Selection Logic (Marquee) ---
  const handleMouseDown = (e: React.MouseEvent) => {
    // Only start box selection if we click on the container background, OR if we click but not on a file-row (handled by bubbling check)
    // Here we attach to the container. If a file item was clicked, e.stopPropagation() in FileTreeItem prevents this.
    
    if (e.button !== 0) return // Only left click

    if (!e.ctrlKey && !e.metaKey) setSelectedIds([]) // Clear if no modifier
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    // === Collision Detection ===
    // Find all file items in the DOM and check overlap
    const fileItems = fileListRef.current.querySelectorAll('.file-item-row')
    const newSelected = new Set(initialSelectedIds)

    fileItems.forEach((el) => {
        const htmlEl = el as HTMLElement
        // Calculate element position relative to the container scroll area
        // element.offsetTop is relative to offsetParent (the container)
  ...
  const FileTreeItem = ({ item, depth }: { item: FileSystemItem, depth: number }) => {
    ...
    return (
      <div className="select-none">
        <div 
          draggable
          // IMPORTANT: data-id and class for collision detection
          data-id={item.id}
          className={clsx(
        ...
        {sidebarView === 'explorer' && (
            <>
                <div className="h-10 px-3 flex items-center justify-between border-b border-[#333] shrink-0">
                    <span className="text-[11px] font-bold uppercase tracking-wider">{vt.explorer}</span>
                    ...
                    {openFiles.map(fid => {
                        ...
                        return (
                            <div key={f.id} onClick={(e)=>{e.stopPropagation(); setActiveFileId(f.id)}} className={clsx("group px-3 flex items-center gap-2 min-w-[120px] max-w-[200px] text-xs border-r border-[#2b2b2b] cursor-pointer select-none", activeFileId===f.id ? "bg-[#1e1e1e] text-white border-t-2 border-t-blue-500" : "text-[#969696]")}>
                                ...
                                <X size={14} className={clsx("opacity-0 hover:bg-[#444] rounded p-0.5", activeFileId===f.id ? "opacity-100" : "group-hover:opacity-100")} onClick={(e) => {
                                    e.stopPropagation()
                    ...
                    <div className={clsx("flex flex-col h-full transition-all duration-300", showPreview ? "w-1/2 border-r border-[#2b2b2b]" : "w-full")}>
                        <div className="flex-1 relative flex">
                            ...
                            <textarea 
                                ...
                                onChange={(e) => setFileSystem(p => p.map(f => f.id === activeFileId ? { ...f, content: e.target.value, isUnsaved: true } : f))}
